...
module.exports = function(app) {
	...
	function matcher(studentProfile, done) {
		...
		app.schooldb.list({include_docs: true}, function(err, schooldocs) {
			...
			_.each(schooldocs.rows, function(school) {
                ...
                // console.log('checking school data:', school.doc._id);
				if (excludeIncompleteSchool(school.doc)) return;
                // issue#1 - exclude schools with incomplete tradeoff data :(
    ...
    function excludeIncompleteSchool(schoolDoc) {
        ...
        // console.log('schoolDoc ok:', schoolDoc._id);
        return false;
		...
		_.each(matches, function(schoolMatch, i){
			...
			_.each(dataFields, function(field, key1){
				_.each(field, function(sub, i){
					...
					if (value) schoolValues[key1 + "." + key2] = value; // the actual number goes here
					if (!value) hadDataGap = true;
				});
			});
			...
			if (!hadDataGap) {
				...
				options.push({
					...
					values: schoolValues,
					description_html: "<p>" + schoolMatch.doc.overview_paragraph + "</p><p><a href='http://insideschools.org/search?q=" + schoolId + "' target='_new'>Read more on InsideSchools.org</a></p>" 
				});	
			} 
		});
// + schoolMatch.id + link to "http://insideschools.org/search?q=" + schoolId
		// console.log("OPTIONS:", options);
		// console.log('compare matches to final matches:', matches.length, finalMatches.length, options.length);
		// return;
		// var options = matches; // 
		// console.log('step - running tradeoffAnalytics.dilemmas');
		var tradeoffData = {columns: columns, subject: "School Comparision", options: options};
		// console.log(tradeoffData);
		app.tradeoffAnalytics.dilemmas(tradeoffData, function(err, tradeoffData){
			...
			app.tradeoffdb.insert(tradeoffData, function(err, ok){
				...
				done(null, {tradeoffId: ok.id, matches: finalMatches}); // just send back the id the tradeoff scripts will query the endpoint by itself
			})
